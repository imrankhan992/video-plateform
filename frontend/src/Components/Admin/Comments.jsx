...
useEffect(() => {
  const getComments = async () => {
    try {
      ...
      const response = await fetch(`${backendURL}/admin/getallcomments`);
      if (!response.ok) {
        throw new Error("Failed to fetch comments");
      }
      const { comments } = await response.json();

      // 🚩 N+1 Warning: Consider optimizing on the backend
      const commentsWithVideoData = await Promise.all(
        comments.map(async (comment) => {
          try {
            ...
            if (!videoResponse.ok) {
              throw new Error(`Failed to fetch video data for videoid ${comment.videoid}`);
            }
            const videoData = await videoResponse.json();

            return {
              ...comment,
              videoData, // add videoData to comment
            };
          } catch (videoError) {
            console.error(videoError);
            return {
              ...comment,
              videoData: null, // fallback if video data fails
            };
          }
        })
      );
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (element.videoData._id !== undefined) {
                            window.location.href = `/admin/studio/video/comments/${element.videoData._id}`;
                          }
                        }}
                      >
                        <img
