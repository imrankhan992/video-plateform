...
import AffiliateAdThumbnailSection from "./AffiliateAdParent/AffiliateAdThumbnailSection";
...
function VideoDetails() {
  ...
  const videolink = "https://example.app/video";
  ...
  const CopiedNotify = () =>
    ...
    // PRODUCT IMAGE
  const [affiliateThumbnailImage, setAffiliateThumbnailImage] = useState(null);
  const [affiliateThumbnailSelected, setAffiliateThumbnailSelected] = useState(false);
  const [affiliateFinalThumbnail, setAffiliateFinalThumbnail] = useState(null);
  const [affiliateFakeLoading, setAffiliateFakeLoading] = useState(false);
  const [affiliateOptionClicked, setAffiliateOptionClicked] = useState(false);
  const [affiliateLink, setAffiliateLink] = useState("");

  const affiliateOptionRef = useRef(null);
const handleAffiliateThumbnailUpload = (e) => {
  const files = Array.from(e.target.files);
  if (files.length > 0) {
    setChanges(true);
    setAffiliateFakeLoading(true);
    setTimeout(() => {
      setAffiliateThumbnailImage(files[0]);
      setAffiliateThumbnailSelected(true);
      setAffiliateFinalThumbnail(files[0]);
      setAffiliateFakeLoading(false);
    }, 500);
  }
};

  const handleAffiliateThumbnailDownload = () => {
    if (!affiliateThumbnailImage) return;
    const link = document.createElement("a");
    link.href = URL.createObjectURL(affiliateThumbnailImage);
    link.download = "affiliate-thumbnail.png";
    link.click();
  };
    ...
    const GetVideoData = async () => {
      try {
        if (id) {
          ...
          setAffiliateThumbnailImage(data.productAdsThumbnail);
  ...
  const UploadThumbnail = async () => {
    ...
    const uploadProductAdsImages = async () => {
    try {
      if (
        !affiliateFinalThumbnail ||
        (videodata && affiliateFinalThumbnail === videodata.affiliateFinalThumbnail)
      ) {
        return videodata.affiliateFinalThumbnail;
      }

      const fileReference = ref(storage, `productAds/${affiliateFinalThumbnail.name}`);
      const uploadData = uploadBytesResumable(fileReference, affiliateFinalThumbnail);
  ...
  const SaveData = async () => {
    try {
      ...
      let uploadProductAdsImg = await uploadProductAdsImages();
      ...
      const data = {
        thumbnail: img,
        productAdsThumbnail: uploadProductAdsImg,
        ...
        <div className="current-editvideo-data">
          <div className="video-details-left">
            <div className="current-video-editable-section">
              ...
              {/* product */}
  <AffiliateAdThumbnailSection
      theme={theme}
      videodata={videodata}
      thumbnailImage={affiliateThumbnailImage}
      setThumbnailImage={setAffiliateThumbnailImage}
      thumbnailSelected={affiliateThumbnailSelected}
      setThumbnailSelected={setAffiliateThumbnailSelected}
      finalThumbnail={affiliateFinalThumbnail}
      setFinalThumbnail={setAffiliateFinalThumbnail}
      fakeLoading={affiliateFakeLoading}
      setFakeLoading={setAffiliateFakeLoading}
      optionClicked={affiliateOptionClicked}
      setOptionClicked={setAffiliateOptionClicked}
      optionRef={affiliateOptionRef}
      handleThumbnailUpload={handleAffiliateThumbnailUpload}
      handleThumbnailDownload={handleAffiliateThumbnailDownload}
      handleAffiliateLinkClick={handleAffiliateLinkClick}
      affiliateLink={affiliateLink}
