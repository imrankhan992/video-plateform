...
Studio.post("/savevideoeditdetails/:videoId", async (req, res) => {
  try {
    const { videoId } = req.params;
    const { thumbnail, title, desc, tags, privacy, productAdsThumbnail, affiliateLink } = req.body;
    ...
    await videodata.updateOne(
      ...
      {
        $set: {
          "VideoData.$.productAdsThumbnail": productAdsThumbnail,
          "VideoData.$.affiliateLink": affiliateLink,
    ...
    await userData.updateMany(
      { "likedVideos.likedVideoID": videoId },
      {
        $set: {
          ...
          "likedVideos.$.productAdsThumbnail": productAdsThumbnail,
          "likedVideos.$.affiliateLink": affiliateLink,
    ...
    await userData.updateMany(
      ...
      {
        $set: {
          "watchLater.$.productAdsThumbnail": productAdsThumbnail,
          "watchLater.$.affiliateLink": affiliateLink,
    ...
    await userData.updateMany(
      ...
      {
        $set: {
          "Playlists.$[playlist].playlist_videos.$[video].productAdsThumbnail": productAdsThumbnail,
          "Playlists.$[playlist].playlist_videos.$[video].affiliateLink": affiliatelink,
...
Studio.get("/admin/getallcomments", async (req, res) => {
  try {
    ...
    // Flatten VideoData from all users
    const allVideoData = video.flatMap((doc) => doc.VideoData || []);

    // Extract comments
    const comments = allVideoData.flatMap((data) => data.comments || []);

    if (comments.length === 0) {
      return res.status(404).json({ error: "No comments found" });
    }

    // Extract videoids from comments
    const videoid = comments.map((item) => item.videoid);
...
Studio.get("/admin/getvideocommentsbyid/:videoId", async (req, res) => {
  try {
    const { videoId } = req.params;
    const video = await videodata.findOne({ "VideoData._id": videoId });
